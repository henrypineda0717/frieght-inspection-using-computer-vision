// scripts.js – full version with video summary updates every 2 seconds
// and dynamic recent files table
...
const btnSaveToDatabase = getElement('btnSaveToDatabase');
...
// 1. Store the result globally
lastAnalysisResult = data;
lastDetections = data.detections || [];
// 2. ENABLE BUTTONS (The critical fix)
// We use the variables defined at the top of your scripts.js
...
// Enable control buttons, disable analyze
...
if (currentSessionId) {
fetch(`${API_BASE}/video-session/${currentSessionId}`, {
method: 'DELETE',
keepalive: true
            }).catch(() => {});
currentSessionId = null;
...
// --- Polling with summary every 2 seconds ---
...
if (!file) return;
stopVideoSession(); // stop any active session
stopAutoAnalyze(); // stop auto-analyze for images
...
// Video mode: extract first frame and display it on canvas
video.style.display = 'none';
canvas.style.display = 'block';
mjpegImg.style.display = 'none';
ctx.clearRect(0, 0, canvas.width, canvas.height);
// Temporary video to grab first frame
...
// Store video file for later session
...
// Disable report/save buttons until new analysis
if (btnDownloadReport) btnDownloadReport.disabled = true;
if (btnSaveToDatabase) btnSaveToDatabase.disabled = true;
        });
    }
...
try {
// The backend expects Form Data, not raw JSON
const fd = new FormData();
// 1. Add the JSON data as a string (matching backend argument 'analysis_data')
fd.append("analysis_data", JSON.stringify(lastAnalysisResult));
// 2. Add the actual image file (matching backend argument 'image')
fd.append("image", currentImageData);
// 3. POST to the correct endpoint: /api/save-analysis
const response = await fetch(`${API_BASE}/api/analyze/save-analysis`, {
method: 'POST',
body: fd // Do NOT set Content-Type header; browser does it for FormData
            });
...
// Load recent inspections on startup
...
// Also reload recent inspections after a successful save
// (We'll modify the save handler to call it)
const originalSaveHandler = btnSaveToDatabase ? btnSaveToDatabase.click : null;
if (btnSaveToDatabase) {
    const originalListener = btnSaveToDatabase.click;
    btnSaveToDatabase.addEventListener('click', async function(e) {
        // If there's already a listener, we need to be careful; we can override by wrapping.
        // However, the existing listener is already attached. Instead, we'll add a new listener
        // that runs after the save is complete. Since we can't easily modify the existing listener,
        // we'll add an additional listener that calls loadRecentInspections after the save.
        // To avoid duplication, we'll store the original and call it, then load.
        // But the original is already attached, so we'll just add another that waits.
        // This is a bit hacky but works because the original does not block.
        // Better: we modify the existing save handler directly. Let's do that.
    });
}
// Instead, we'll override the save handler to include the reload.
// Remove the old listener and add a new one that includes the reload.
if (btnSaveToDatabase) {
    // Remove any existing listeners (we assume there's only one – the one defined above)
    // A simple way is to replace the onclick or use a new listener and prevent default.
    // We'll just redefine the handler by removing all and adding our own.
    const newSaveHandler = async () => {
        ...
        try {
            ...
            // Reload recent inspections after successful save
    ...
    const newBtn = btnSaveToDatabase.cloneNode(true);
    btnSaveToDatabase.parentNode.replaceChild(newBtn, btnSaveToDatabase);
    // Update global reference
    btnSaveToDatabase = newBtn;
    btnSaveToDatabase.addEventListener('click', newSaveHandler);
}
