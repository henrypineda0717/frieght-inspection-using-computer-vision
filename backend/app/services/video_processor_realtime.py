...
class RealtimeVideoProcessor:
    ...
    def __init__(
        ...
        # NEW ATTRIBUTES for defect persistence
        self.persistence_service = None        # to be set from outside
        self.inspection_stage = None            # passed from API
        self.session_id = None                   # optional
        self.saved_track_ids = set()             # tracks already saved
    ...
    #                     image=frame, view_type=view_type
    #                 )
    ...
    #     """
    #     Processing thread: Applies tracking only when detections exist.
    #     Skips gap-filling/prediction to avoid 'ghost' boxes.
    #     """
    ...
    #             # ONLY proceed if there are actual detections from the model
    #             if detections and len(detections) > 0:
    #                 display_detections = detections.copy()

    #                 if self.tracker is not None:
    #                     # 1. Prepare detections for tracker [x1, y1, x2, y2, conf, class_id]
    #                     dets = []
    ...
    #                         x2 = x1 + det['bbox_w']
    #                         y2 = y1 + det['bbox_h']
    #                         dets.append([x1, y1, x2, y2, det['confidence'], 0])

    #                     try:
    #                         # 2. Update tracker with current detections
    #                         tracks = self.tracker.update(np.array(dets), frame)
                            
    #                         # 3. Match tracks back to raw detections
    #                         # We iterate detections and find the best track for each (IOU or Distance)
    #                         for i, det in enumerate(display_detections):
    #                             det_center = (det['bbox_x'] + det['bbox_w'] / 2,
    #                                         det['bbox_y'] + det['bbox_h'] / 2)
                                
    #                             best_track_id = None
    #                             min_dist = float('inf')

    #                             for track in tracks:
    #                                 # tracker format: [x1, y1, x2, y2, track_id, conf, cls]
    #                                 tx1, ty1, tx2, ty2, t_id = track[:5]
    #                                 track_center = ((tx1 + tx2) / 2, (ty1 + ty2) / 2)
                                    
    #                                 dist = (det_center[0] - track_center[0])**2 + (det_center[1] - track_center[1])**2
    #                                 if dist < min_dist:
    #                                     min_dist = dist
    #                                     best_track_id = int(t_id)

    #                             # Assign the ID if a match was found
    #                             if best_track_id is not None:
    #                                 display_detections[i]['track_id'] = best_track_id
    ...
    #                         if tid is not None and tid not in self.saved_track_ids:
    #                             self.persistence_service.persist_defect(
    #                                 frame_bgr=frame,
    #                                 defect=det,
    #                                 container_id=self.stable_container_id,
    #                                 frame_number=frame_number,
    ...
    #                             )
    #                             self.saved_track_ids.add(tid)

    #             # Update thread-safe reference for UI
    #             with self._detections_lock:
    #                 self.current_detections = display_detections

    #             # Container ID Stability Logic
    #             current_id = self._extract_best_container_id(display_detections)
    ...
    def _run_inference(self):
            """
            Inference thread: Run detection every N frames.
            Gap frames are completely skipped and discarded to ensure the UI
            only receives frames that were actually processed.
            """
            logger.info("Inference thread started")
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    ...
                    if should_detect:
                        ...
                        # Push ONLY processed frames to detection queue
                        self.detection_queue.put({
                            'frame': frame,
                            'frame_number': frame_number,
                            'detections': enriched_detections,
                    ...
                    # Notice: The 'else' block is completely removed. 
                    # Gap frames are discarded and never sent to processing.
    ...
    def _process_results(self):
            """
            Processing thread: Applies tracking and persistence.
            Guarantees tracks are assigned so defects can be saved properly.
            """
            logger.info("Processing thread started")
            ...
            while not self.stop_event.is_set():
                ...
                try:
                    ...
                    # ONLY proceed if there are actual detections
                    if detections and len(detections) > 0:
                        display_detections = detections.copy()
                        ...
                        # --- PERSISTENCE (Saving Defects) ---
                        # if self.persistence_service:
                        #     for det in display_detections:
                        #         tid = det.get('track_id')
                        #         # Only save if we successfully generated an ID and haven't saved it yet
                        #         if tid is not None and tid not in self.saved_track_ids:
                        #             try:
                        #                 self.persistence_service.persist_defect(
                        #                     frame_bgr=frame,
                        ...
                        #                 logger.error(f"Failed to save defect track {tid}: {save_err}")
                        # --- PERSISTENCE (Saving Defects) ---
                        ...
                        if self.persistence_service:
                            for det in display_detections:
                                ...
                                # Only save if track not already saved
                                if tid not in self.saved_track_ids:
                                    model_source = det.get('model_source', '').lower()
                                    container_id_val = det.get('container_id') or 'UNKNOWN'
                                    

                                    # For General detections, require a valid container ID
                                    if model_source == 'General':
                                        if not container_id_val or container_id_val == 'UNKNOWN':
                                            print(f"Track {tid} (General) skipped: no valid container ID")
                                            continue
                                    print(container_id_val)
                                    print()
                                    print()
                                    print('-------saving defects -------------')
                                    print()
                                    
                                    # For all others (damage, etc.), proceed
                                    try:
                                        self.persistence_service.persist_defect(
                                            frame_bgr=frame,
                                            defect=det,
                                        ...
                                        self.saved_track_ids.add(tid)
                                        logger.info(f"✅ Saved track {tid} (model={model_source}, container={container_id_val})")
                                    except Exception as save_err:
                                        logger.error(f"Failed to save defect track {tid}: {save_err}")

                    # Update thread-safe reference for UI polling
                    with self._detections_lock:
                        self.current_detections = display_detections
    ...
    def process_video(
        ...
        self.stable_container_id = "UNKNOWN"
        self.saved_track_ids.clear()  # reset for new session
        if self.persistence_service:
            self.persistence_service._current_inspection_id = None  # reset for new session
        ...
        try:
            while not self.stop_event.is_set():
                ...
                try:
                    ...
                    logger.info(f"🎨 Drawing frame {frame_number} with {len(detections)} detections")
            ...
            # Finalize inspection if persistence service exists
            if self.persistence_service:
                summary = self.get_current_summary()
                self.persistence_service.finalize_inspection(
                    final_contamination=summary.get('contamination_index'),
    ...
    def draw_detections(self, frame: np.ndarray, detections: List[Dict[str, Any]]) -> np.ndarray:
            ...
            """
            logger.info(f"✏️ draw_detections called with {len(detections)} detections")
            if detections:
                    logger.info(f"   First detection keys: {detections[0].keys()}")
                    logger.info(f"   First detection sample: {detections[0]}")
            if not detections:
                return frame
