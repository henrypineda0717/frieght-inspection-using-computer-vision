...
async def start_video_session(
    ...
    - Creates a database session and persistence service for saving defects.
    ...
    try:
        ...
        # 3. Create database session and persistence service (NEW)
        try:
            db = SessionLocal()
        ...
        # 5. Attach persistence and session metadata (NEW)
        processor.persistence_service = persistence
        processor.inspection_stage = params.inspection_stage
        processor.session_id = str(uuid.uuid4())   # you can also reuse the same session_id

        # 6. Store Session Metadata (include db and persistence for cleanup)
        ...
        sessions[session_id] = {
            ...
            "db": db,                         # store for later cleanup
            "persistence": persistence,        # optional, but useful
        }
        ...
        # Close database session if created
        if db:
            db.close()
...
def _cleanup_session_resources(session_id: str):
    ...
    if session:
        ...
        # Close database session (NEW)
        db = session.get("db")
        if db:
            try:
                db.close()
                logger.info(f"Closed database session for {session_id}")
            except Exception as e:
